...
class ServerLauncherSettings():
    def __init__(self):
        ...
        self.translate = {
            '通过JMX检测服务器每刻耗时':'enable-jmx-monitoring',
            'RCON远程访问端口':'rcon.port',
            '新玩家的游戏模式':'gamemode',
            '启用命令方块':'enable-command-block',
            '允许使用GameSpy4协议的服务器监听器，用于获取服务器信息':'enable-query',
            '世界名称及存档文件夹名称':'level-name',
            '玩家客户端在多人游戏服务器列表中现实的服务器信息':'motd',
            '设置监听服务器的端口号':'query.port',
            '开启PVP':'pvp',
            '游戏难度':'difficulty',
            '服务器数据包压缩限额':'network-compression-threshold',
            '强制使用服务器提供的材质包':'require-resource-pack',
            '每tick最长时间（超时看门狗将会强制关闭服务器）':'max-tick-time',
            '使用针对Linux平台的数据包收发优化':'use-native-transport',
            '服务器同时容纳最大玩家数量':'max-players',
            '是否查验玩家登录验证信息(是否允许盗版玩家登录)':'online-mode',
            '是否使服务器在服务器列表中显示为在线的':'enable-status',
            '允许玩家在安装添加飞行功能的mod前提下在生存模式飞行':'allow-flight',
            '向所有在线OP发送通知RCON执行的命令输出':'broadcast-rcon-to-ops',
            '设置服务端发送给客户端的世界数据量，也就是设置玩家各个方向上的区块数量（半径）决定服务器的视距':'view-distance',
            '将服务器与一个特定IP绑定，建议留空':'server-ip',
            '用于在强制使用服务器材质包时在资源包提示界面显示自定义信息（可包含多行文本）':'resource-pack-prompt',
            '允许玩家进入下界':'allow-nether',
            '改变服务器监听的端口号':'server-port',
            '是否允许远程访问服务器控制台':'enable-rcon',
            '是否以同步模式写入区块文件':'sync-chunk-writes',
            '设置使用/op命令式OP的权限等级':'op-permission-level',
            '如果服务器发送的ISP/AS和Mojang的验证服务器的不一样，玩家就会被踢出':'prevent-proxy-connections',
            '可输入指向一个资源包的URL,玩家可选择是否使用改资源包（应在:和=钱添加反斜线）':'resource-pack',
            '控制实体需要距离玩家多近才会将数据包发给客户端':'entity-broadcast-range-percentage',
            'RCON远程访问的密码':'rcon.password',
            '玩家挂机自动踢出时间':'player-idle-timeout',
            '玩家重新登录时强制设为默认游戏模式':'force-gamemode',
            '玩家被踢出服务期前，可以发送的数据包数量':'rate-limit',
            '忽略难度设置设为hard，死后自动切换为旁观模式(单机极限模式)':'hardcore',
            '启用服务器的白名单，op无需加入':'white-list',
            '向所有在线OP发送执行命令的输出':'broadcast-console-to-ops',
            '是否生成村民':'spawn-npcs',
            '是否生成动物':'spawn-animals',
            '是否允许服务端定期发送统计数据到http://snoop.minecraft.net':'snooper-enabled',
            '设定函数的默认权限模式':'function-permisiion-level',
            '是否生成攻击性生物':'spawn-monsters',
            '在服务器上强制执行白名单':'enforce-whitelist',
            '资源包的SHA-1值，必须使用小写十六进制，用于验证资源包的完整性':'resource-pack-sha1',
            '保护出生点（2x+1）':'spawn-protection',
            '设置可让世界边界获得的最大半径值':'max-world-size',
        }
